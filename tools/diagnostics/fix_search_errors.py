...
def is_likely_wrong_actress_data(actresses: List[str]) -> bool:
    ...
    # 檢查是否包含明顯的非女優關鍵詞
    wrong_keywords = [
        'シロウト', 'しろうと', 'パコパコ', 'ナンパ', 'ハメ撮り',
        'ギャラリー', 'チャンネル', 'ドリームチケット', 'レーベル',
        'プロジェクト', 'グループ', 'クラブ', 'サークル',
        '素人', '企画', '配信', '限定', 'レズれ', 'ミラー号',
        'れいんぎが', 'ぎが', 'かぐや', 'ちゃん', 'さん'
    ]
    
    # 如果超過一半的"女優"包含錯誤關鍵詞
    wrong_count = sum(
        1 for actress in actresses 
...
def fix_database(db_path: str, backup: bool = True):
    ...
    # 備份
    if backup:
        backup_path = db_file.parent / f"data_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def show_specific_video(db_path: str, code: str):
    ...
    # 嘗試多種格式
    video_info = None
    for key in [code, code.upper(), code.replace('-', ''), code.upper().replace('-', '')]:
        if key in videos:
