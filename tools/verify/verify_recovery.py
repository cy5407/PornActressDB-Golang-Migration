...
async def test_failed_codes_recovery():
    ...
    # 從用戶提供的警告日誌中提取的失敗番號
    failed_codes = [
        'FSDSS-180', 'SNIS-515', 'STARS-814', 'SONE-709', 'EBWH-118',
        'SSIS-592', 'ABP-698', 'MIDV-905', 'MDTM-793', 'EBWH-111',
        'SSIS-625', 'SSIS-820', 'ABP-835', 'CAWD-437', 'START-193',
        'SNIS-589', 'SSIS-324', 'SSIS-808', 'STARS-818', 'MVSD-615',
        'STARS-918', 'STARS-859', 'CAWD-773', 'FSDSS-931', 'MIDV-676',
        'START-018', 'ABF-137', 'SONE-324', 'SONE-982', 'MIDV-742',
        'ABF-133', 'MIDV-743', 'SSIS-535', 'SSIS-617', 'SONE-040',
        'MIDA-348', 'SQTE-416', 'EBWH-138', 'SONE-238', 'MIDV-747',
        'SNIS-939', 'SONE-114', 'ABP-592', 'SONE-065', 'ADN-593',
        'FSDSS-855', 'STARS-727', 'STARS-734', 'SONE-544', 'SNIS-731',
    ]
    ...
    for i, code in enumerate(failed_codes, 1):
        try:
            ...
            if actresses:
                ...
                print(f"✅ [{i:2d}] {code}: {len(actresses):2d} 位女優 - {actresses[0]}", end='')
                if len(actresses) > 1:
    ...
    print(f"✅ 成功恢復: {success_count} 個 ({success_count/len(failed_codes)*100:.1f}%)")
    print(f"❌ 仍然失敗: {failed_count} 個 ({failed_count/len(failed_codes)*100:.1f}%)")
