...
try:
    ...
    spec = importlib.util.spec_from_file_location(
        "web_searcher", Path(__file__).parent / "src" / "services" / "web_searcher.py"
    )
...
def test_batch_search_avwiki():
    ...
    results = searcher.batch_search_avwiki_concurrent(
        BATCH_TEST_CODES, stop_event, progress_callback=progress_cb
    )
    ...
    print("=" * 70)
    print(
        f"成功: {success_count}/{len(BATCH_TEST_CODES)} ({success_count / len(BATCH_TEST_CODES) * 100:.1f}%)"
    )
    print(
        f"失敗: {failure_count}/{len(BATCH_TEST_CODES)} ({failure_count / len(BATCH_TEST_CODES) * 100:.1f}%)"
    )
