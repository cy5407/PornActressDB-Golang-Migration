...
async def test_regression():
    ...
    print("回歸測試結果")
    print("=" * 70)
    print(f"成功: {success_count}/{len(REGRESSION_TEST_CODES)} ({success_count/len(REGRESSION_TEST_CODES)*100:.1f}%)")
    print(f"失敗: {failure_count}/{len(REGRESSION_TEST_CODES)} ({failure_count/len(REGRESSION_TEST_CODES)*100:.1f}%)")
