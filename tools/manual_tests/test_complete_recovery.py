...
async def test_all_codes():
    ...
    for i, code in enumerate(TEST_VIDEO_CODES, 1):
        ...
        try:
            ...
            if actresses:
                print(f"✓ {len(actresses)} 位 ({', '.join(actresses[:2])}{'...' if len(actresses) > 2 else ''})")
                success_count += 1
    ...
    print(f"成功: {success_count} 個 ({success_count/len(TEST_VIDEO_CODES)*100:.1f}%)")
    print(f"失敗: {failure_count} 個 ({failure_count/len(TEST_VIDEO_CODES)*100:.1f}%)")
    print()
