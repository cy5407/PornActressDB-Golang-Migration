...
class TestActressStatistics:
    ...
    def db_manager(self):
        ...
        db.add_or_update_actress(
            {"id": "actress_1", "name": "山田美優", "video_count": 0}
        )
        db.add_or_update_actress(
            {"id": "actress_2", "name": "佐藤愛", "video_count": 0}
        )
        db.add_or_update_actress(
            {"id": "actress_3", "name": "鈴木花", "video_count": 0}
        )
...
class TestStatisticsPerformance:
    ...
    def large_db_manager(self):
        ...
        for i in range(50):
            db.add_or_update_actress(
                {"id": f"actress_{i}", "name": f"女優{i}", "video_count": 0}
            )
