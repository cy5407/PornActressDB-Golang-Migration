...
def test_incremental_db():
    ...
    for i in range(10):
        db.update_video(test_code, {
            'title': f'測試標題 {i+1}',
            'test_field': f'測試值 {i+1}'
        })
    elapsed = time.perf_counter() - start
    
    logger.info(f"✅ 完成 10 次快速更新: {elapsed*1000:.2f}ms ({elapsed/10*1000:.2f}ms/次)")
    ...
    logger.info(f"  完成 10 次更新: {standard_elapsed*1000:.2f}ms ({standard_elapsed/10*1000:.2f}ms/次)")
    ...
    logger.info(f"  完成 10 次更新: {incremental_elapsed*1000:.2f}ms ({incremental_elapsed/10*1000:.2f}ms/次)")
