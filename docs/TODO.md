...
**問題**: 多個模組重複執行 `sys.path.insert(0, ...)`，且部分路徑計算錯誤

**修正內容**:
- 建立統一的路徑設定工具 `src/utils/path_setup.py`
- 移除 src/ 中不必要的 sys.path 操作（2 個檔案）
  - `src/services/classifier_core.py`
  - `src/services/web_searcher.py`
- 修正 tools/ 腳本的路徑計算錯誤（24 個檔案）
  - 工具腳本使用正確的 `.parent.parent.parent` 回到專案根目錄
  - 建立批次修正腳本 `tools/fix_sys_path.py` 自動化處理
- 保留部分腳本已正確的路徑設定（如 `tools/analysis/normalize_studios.py`）
...
**狀態**: ✅ 部分完成（透過 Ruff 自動修復）
**預估工作量**: 1-2 小時

**修正步驟**:
1. ~~使用 `pylint` 或 `flake8` 掃描~~ → 使用 Ruff
2. ~~刪除或實現 TODO 標記的程式碼~~

**Ruff 修復結果**:
- 自動修復：3522 個問題
- 剩餘建議：125 個（主要是風格建議，不影響功能）
...
**完成內容**:
1. ✅ 安裝並配置 **Ruff** (2024-2025 主流工具)
2. ✅ 建立 `pyproject.toml` 配置檔
3. ✅ 格式化 77 個檔案
4. ✅ 建立 `Skills.md` 技術文件

**使用工具**: Ruff (取代 Black + Flake8 + isort)
- 速度：比 Black 快 10-100x
- 月下載量：~1.1 億次 (PyPI 2025/12)
- GitHub Stars：44.4k+
...
#### 第三週（已完成）
- [x] 問題 9: 未使用的變數和函式（透過 Ruff 自動修復）
- [x] 問題 10: 代碼風格不一致（Ruff 格式化 77 檔案）

#### 第四週（已完成）
- [x] 問題 7: 重複的 sys.path 操作（修正 24 個工具腳本）
