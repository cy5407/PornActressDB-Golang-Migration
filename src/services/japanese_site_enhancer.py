    ...
    def create_enhanced_soup(
        ...
        for encoding in encoding_priority:
            try:
                ...
                current_best_score = (
                    2 if best_soup and best_soup.find() else 0
                ) - min_replacement_ratio

                if (
                    quality_score > current_best_score
                    or replacement_ratio < min_replacement_ratio
                ):
        ...
        logger.info(
            f"[{url}] 日文網站使用最佳編碼: {best_encoding} (替換比例: {min_replacement_ratio:.3f})"
        )
        return best_soup

    def smart_decode_response(
        self, response: httpx.Response, url: str = ""
    ) -> tuple[str, str]:
        """
        智慧解碼日文網站的 HTTP 回應內容
        ...
        for encoding in encoding_priority:
            try:
                ...
                quality_score = html_quality - replacement_ratio
                current_best_score = (
                    2 if best_content and "<html" in best_content.lower() else 0
                ) - min_replacement_ratio
...
def create_japanese_soup(
    url: str, timeout: int = 10, max_retries: int = 3
) -> BeautifulSoup | None:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(
                f"HTTP 錯誤 (嘗試 {attempt + 1}/{max_retries}): {e.response.status_code} - {url}"
            )
