...
class UnifiedCacheManager:
    ...
    def __init__(self, config=None):
        ...
        if config:
            try:
                ...
                self.max_cache_size_mb = config.getint('cache', 'max_size_mb', fallback=500)
            except Exception:
                pass
        
        logger.info(f"🔗 統一快取管理器已初始化 (TTL: {self.default_ttl_days} 天, 大小限制: {self.max_cache_size_mb} MB)")
    ...
    def get(self, key: str, source: str = None) -> Optional[Any]:
        """
        從快取取得值
    ...
    def cleanup_all(self, ttl_days: int = None, max_size_mb: int = None) -> Dict[str, Any]:
        """
        清理所有快取來源
        ...
        if results['total_deleted'] > 0:
            logger.info(f"🧹 快取清理完成: 共刪除 {results['total_deleted']} 項，釋放 {results['total_freed_mb']:.2f} MB")
        
        return results
    
    def _cleanup_single_source(self, name: str, cache: Any, ttl_days: int, max_size_mb: int) -> Dict[str, Any]:
        """清理單一快取來源"""
    ...
    def _get_source_stats(self, cache: Any) -> Dict[str, Any]:
        ...
        if hasattr(cache, 'get_stats'):
            ...
            return {
                'entries': full_stats.get('memory_cache_entries', 0) + len(full_stats.get('index_entries', [])),
                'size_mb': full_stats.get('total_size_mb', 0.0),
                'memory_entries': full_stats.get('memory_cache_entries', 0),
                'disk_entries': len(full_stats.get('index_entries', [])) if isinstance(full_stats.get('index_entries'), list) else 0,
