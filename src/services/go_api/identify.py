...
def identify_studio(
    ...
    *,
    ...
    try:
        ...
        result = r.run(cmd)
        return r.parse_json(result.stdout)
    except Exception as e:
        logger.error(f"❌ 識別片商失敗: {e}")
        return {"code": code, "studio": "UNKNOWN"}


def identify_studios_batch(
    codes: list[str],
    check_major: bool = False,
    *,
    runner: GoCommandRunner | None = None,
...
def list_studios(*, runner: GoCommandRunner | None = None) -> list[str]:
    ...
    r = _get_runner(runner)
    try:
        result = r.run(["identify", "-list", "-json"])
...
def get_studio_prefixes(
    ...
    *,
    ...
    r = _get_runner(runner)
    try:
        result = r.run(["identify", "-prefixes", "-json", studio_name])
        data = r.parse_json(result.stdout)
