...
def identify_studio(code: str, check_major: bool = False, *, runner: GoCommandRunner | None = None) -> dict:
    ...
    try:
        ...
        return runner.parse_json(result.stdout)
...
def list_studios(*, runner: GoCommandRunner | None = None) -> list[str]:
    ...
    if runner is None:
        runner = _get_bridge()._runner
    try:
        result = runner.run(["identify", "-list", "-json"])
...
def get_studio_prefixes(studio_name: str, *, runner: GoCommandRunner | None = None) -> list[str]:
    ...
    if runner is None:
        runner = _get_bridge()._runner
    try:
        result = runner.run(["identify", "-prefixes", "-json", studio_name])
        data = runner.parse_json(result.stdout)
