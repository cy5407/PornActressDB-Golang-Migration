...
class UnifiedCodeExtractor:
    ...
    def extract_code(self, filename: str) -> str | None:
        ...
        # 移除常見的品質和編碼標記
        cleaned_name = re.sub(
            r"[-_]?[CHch]\d*$", "", cleaned_name
        )  # 移除 -C, CH, -C1 等結尾
        cleaned_name = re.sub(
            r"\.H265$", "", cleaned_name, flags=re.IGNORECASE
        )  # 移除 .H265
        cleaned_name = re.sub(
            r"[-_]?(1080p|720p|4K|HDR|HEVC|AVC|X264|X265)",
            "",
            cleaned_name,
            flags=re.IGNORECASE,
        )
        ...
        # 移除網站標記
        cleaned_name = re.sub(
            r"^(hhd800\.com@|xxx\.com-)", "", cleaned_name, flags=re.IGNORECASE
        )
