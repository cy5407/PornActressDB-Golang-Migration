    ...
    def smart_decode(self, content: bytes, url: str = "") -> tuple[str, str]:
        ...
        for encoding in self.encoding_priority:
            try:
                ...
                replacement_ratio = (
                    replacement_count / total_chars if total_chars > 0 else 1.0
                )

                logger.debug(
                    f"[{url}] {encoding}: {replacement_count} 替換字符, 比例: {replacement_ratio:.3f}"
                )
        ...
        if best_content is None:
            ...
            logger.info(
                f"[{url}] 使用最佳編碼: {best_encoding} (替換比例: {min_replacement_ratio:.3f})"
            )
...
class ImprovedScraper:
    ...
    def scrape(self, url: str, validate_content: bool = True) -> dict[str, Any] | None:
        """
        爬取網頁內容
        ...
        try:
            ...
            # 智慧解碼
            content, encoding = self.encoding_handler.smart_decode(
                response.content, url
            )

            # 驗證內容（如果啟用）
            if validate_content and not self.encoding_handler.validate_content(
                content, url
            ):
    ...
    def extract_video_info(self, result: dict[str, Any]) -> dict[str, Any] | None:
        """
        從爬取結果中提取影片資訊
...
if __name__ == "__main__":
    ...
    for url in test_urls:
        ...
        if result:
            logger.info(
                f"成功: 編碼={result['encoding']}, 內容長度={result['content_length']}, 替換字符={result['replacement_chars']}"
            )
        else:
