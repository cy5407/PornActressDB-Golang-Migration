...
class AsyncWebScraper:
    ...
    def __init__(
        self, config: ScrapingConfig = None, cache_manager: CacheManager = None
    ):
        ...
        logger.info(
            f"🚀 AsyncWebScraper 已初始化 - 最大併發: {self.config.max_concurrent}"
        )
    ...
    async def _make_request(
        ...
        try:
            ...
            async with session.get(
                ...
                # 自動編碼檢測
                decoded_content, encoding = self.encoding_detector.detect_and_decode(
                    content_bytes
                )
    ...
    def _update_stats(
        self, domain: str, success: bool, response_time: float, encoding: str = None
    ):
        """更新統計資訊"""
...
class BatchWebScraper:
    ...
    def scrape_in_batches(
        ...
        for i in range(0, len(urls), self.batch_size):
            ...
            if progress_callback:
                progress_callback(
                    f"處理批次 {batch_num}/{total_batches} ({len(batch_urls)} 個URL)..."
                )
