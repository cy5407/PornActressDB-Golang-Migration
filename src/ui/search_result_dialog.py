...
class SearchResultDialog:
    ...
    def _setup_ui(self):
        ...
        ttk.Label(header_frame, text=stats_text, font=("Microsoft JhengHei", 12)).pack(
            side="right"
        )
        ...
        for col_id, col_text, col_width in column_config:
            self.tree.heading(
                col_id, text=f"{col_text} ▼", command=lambda c=col_id: self._sort_by(c)
            )
        ...
        ttk.Button(button_frame, text="📥 匯出 CSV", command=self._export_csv).pack(
            side="left", padx=5
        )

        ttk.Button(
            button_frame, text="📋 複製失敗番號", command=self._copy_failed
        ).pack(side="left", padx=5)

        ttk.Button(
            button_frame, text="📋 複製成功番號", command=self._copy_success
        ).pack(side="left", padx=5)

        ttk.Button(
            button_frame, text="📋 複製選取項目", command=self._copy_selected
        ).pack(side="left", padx=5)

        ttk.Button(button_frame, text="關閉", command=self._on_close).pack(
            side="right", padx=5
        )
    ...
    def _insert_item(self, code: str, result: SearchResultItem) -> str:
        ...
        if result.actresses:
            if len(result.actresses) > 3:
                actresses_str = (
                    ", ".join(result.actresses[:3]) + f" (+{len(result.actresses) - 3})"
                )
            else:
    ...
    def _show_detail_dialog(self, code: str, result: SearchResultItem):
        ...
        # 番號
        ttk.Label(frame, text="番號:", font=("Microsoft JhengHei", 10, "bold")).grid(
            row=0, column=0, sticky="w", pady=2
        )
        ttk.Label(frame, text=code).grid(row=0, column=1, sticky="w", pady=2)

        # 狀態
        ttk.Label(frame, text="狀態:", font=("Microsoft JhengHei", 10, "bold")).grid(
        ...
        # 女優
        ttk.Label(frame, text="女優:", font=("Microsoft JhengHei", 10, "bold")).grid(
        ...
        # 來源
        ttk.Label(frame, text="來源:", font=("Microsoft JhengHei", 10, "bold")).grid(
            row=3, column=0, sticky="w", pady=2
        )
        ttk.Label(frame, text=result.source or "-").grid(
            row=3, column=1, sticky="w", pady=2
        )

        # 片商
        ttk.Label(frame, text="片商:", font=("Microsoft JhengHei", 10, "bold")).grid(
            row=4, column=0, sticky="w", pady=2
        )
        ttk.Label(frame, text=result.studio or "-").grid(
        ...
        if result.tried_sources:
            ttk.Label(
                frame, text="嘗試來源:", font=("Microsoft JhengHei", 10, "bold")
            ).grid(row=5, column=0, sticky="w", pady=2)
            ttk.Label(frame, text=" → ".join(result.tried_sources)).grid(
                row=5, column=1, sticky="w", pady=2
            )

        # 關閉按鈕
        ttk.Button(frame, text="關閉", command=detail_window.destroy).grid(
    ...
    def _copy_to_clipboard(self, codes: list[str], label: str):
        ...
        if codes:
            ...
            messagebox.showinfo(
                "已複製",
                f"已複製 {len(codes)} 個{label}番號到剪貼簿",
                parent=self.dialog,
            )
        else:
...
def show_search_results(
    parent: tk.Tk, results: dict[str, Any], title: str = "搜尋結果預覽"
) -> None:
    """
    顯示搜尋結果預覽對話框的便利函式
