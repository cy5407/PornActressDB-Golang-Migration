...
### 輸入
- 女優名稱（從 searchResults 中取得，actresses[0]）
...
| 無女優資訊 | 番號無女優記錄，或後端返回 "" | `未分類\` |

### 判定流程

```
1. 從 searchResults 取番號對應的女優名（actresses[0]）
   → 無女優 → 放到「未分類\番號.ext」

2. 前端批次呼叫 GetActressPrimaryStudios(actressNames)
   → 後端掃描 DB，統計每位女優的片商作品數

3. GetActressPrimaryStudio 取作品數最多的片商（max_studio）
   → 若多個片商作品數相同，取名稱字典序較小者
   → 空片商或 "UNKNOWN" 忽略

4. 後端判斷 max_studio 是否在 major_studios.json 中
   → 是 → 返回片商名（如 "S1"）
   → 否 → 返回 "單體企劃女優"
   → 無任何記錄 → 返回 ""（前端歸入未分類）
```
...
async function handleStudioMove() {
  // 1. 從 searchResults 建立 codeToActress 映射
  // 2. 批次呼叫 GetActressPrimaryStudios(actressNames)
  // 3. 依返回值計算目標路徑：
  //    "" → outputDir\未分類\番號.ext
  //    "S1" → outputDir\S1\女優名\番號.ext
  //    "單體企劃女優" → outputDir\單體企劃女優\女優名\番號.ext
  // 4. 呼叫 BatchMove
  // 5. 成功後清除 scanResults 已移動項目
}
```
...
| 🏢 片商分類 | `outputDir\片商\女優名\番號.ext` | 查 DB 統計 + major_studios |

## 相關檔案

- `major_studios.json` — 大片商清單
- `pkg/database/jsondb.go` — `GetActressPrimaryStudio` DB 查詢層
- `wails-app/backend/app.go` — `GetActressPrimaryStudios` Wails binding
- `wails-app/frontend/src/App.tsx` — `handleStudioMove` + 按鈕
- `wails-app/frontend/wailsjs/go/backend/App.js/d.ts` — Wails 自動生成 stubs
